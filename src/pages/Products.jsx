...
import { Search, Plus, Edit2, Trash2, Globe, Tag, X } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { getAllProducts, searchProducts } from '../lib/products/productService';
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden animate-in fade-in slide-in-from-top-4 duration-700">
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100">
                ...
                <th className="px-6 py-4 text-[11px] font-bold text-slate-400 uppercase tracking-widest">Category & Tags</th>
                  ...
                  <td className="px-6 py-5">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex flex-col">
                        <span className="text-sm font-bold text-slate-800 tracking-tight">{prod.title}</span>
                        <span className="text-[11px] text-slate-400 font-bold uppercase tracking-widest mt-1">ID: PR-00{prod.id}</span>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-5">
                    <div className="flex flex-col gap-2">
                      <span className="text-xs font-bold text-slate-700 uppercase tracking-wide bg-slate-50 px-2.5 py-1.5 rounded-lg border border-slate-100 w-fit">{prod.category}</span>
                      <div className="flex flex-wrap gap-1.5">
                        {prod.tags.map((t, index) => (
        ...
        <div className="bg-slate-50/50 px-8 py-4 border-t border-slate-100 flex items-center justify-between">
          <span className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Showing {totalReceivedCount} out of {totalEntriesCount} products</span>
